...
To install the chart with the release name `nalbam`:
...
```bash
$ helm install --name nalbam -f values.yaml stable/jenkins
```
...
```bash
$ helm install --name nalbam --set Persistence.ExistingClaim=PVC_NAME stable/jenkins
```
